...
class CaptchaSolver:
    """비전 모델 기반 CAPTCHA 해결기 - 로컬 모델만 사용"""

    def __init__(self, vision_model, claude_client=None):
        self.vision_model = vision_model
        self.claude = claude_client  # 사용 안함 (로컬만)
        self.solved_count = 0
        self.failed_count = 0
    ...
    async def detect_captcha(self, page) -> dict:
        ...
        captcha_info = await page.evaluate("""
            () => {
                ...
                const turnstile = document.querySelector('iframe[src*="challenges.cloudflare.com"], iframe[src*="turnstile"]');
                if (turnstile) {
                    result.found = true;
                ...
                if (recaptcha) {
                    ...
                    result.x = rect.x + 30;  // 체크박스는 왼쪽에 있음
                    result.y = rect.y + rect.height / 2;
                    return result;
                }
                ...
                if (document.body.innerText.includes('Checking your browser') ||
                    ...
                    result.found = true;
                    result.type = 'cloudflare_wait';
                    return result;
                }

                // 일반 체크박스
                const checkbox = document.querySelector('.cf-turnstile input, input[type="checkbox"]');
                if (checkbox) {
